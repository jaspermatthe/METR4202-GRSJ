...
class MapExplorer(Node):
    ...
    # Check navigation status and call frontier finder when IDLE
    print("in bt callback")
    for event in msg.event_log:
        if event.node_name == 'NavigateRecovery' and event.current_status == 'IDLE':
            print("Found out that NavigateRecovery is IDLE")
            self.frontier_finder()  # Trigger the frontier finder
